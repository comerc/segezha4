...
func screenshotWithPopup(sel string, buf *[]byte) func(context.Context) error {
	return func(ctx context.Context) error {
		if err := chromedp.Screenshot(sel, buf, chromedp.NodeVisible).Do(ctx); err != nil {
			return err
		}
		if isPopup, err := hidePopup(ctx); err != nil {
			return err
		} else if isPopup {
			if err := chromedp.Screenshot(sel, buf, chromedp.NodeVisible).Do(ctx); err != nil {
				return err
...
func hidePopup(ctx context.Context) (bool, error) {
	selPopup := "body #gtm_popup_blocker_iframe"
	var nodes []*cdp.Node
	if err := chromedp.Nodes(selPopup, &nodes, chromedp.AtLeast(0)).Do(ctx); err != nil {
		return false, err
	}
	if len(nodes) == 0 {
		return false, nil
	}
	if err := chromedp.SetAttributeValue(selPopup, "style", "display:none").Do(ctx); err != nil {
		return false, err
	}
	return true, nil
}
